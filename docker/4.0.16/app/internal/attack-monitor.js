...
async function applyPendingReload(knex) {
	if (!pendingReload) return;
	const nowMs = Date.now();
	if (nowMs - lastReloadMs <= 5_000) return;
	lastReloadMs = nowMs;
	pendingReload = false;
	try {
...
function isLoggableAttackType(t) {
	return isValidAttackType(t) || t === "deny" || t === "country_deny";
}
...
function parseJsonLine(line) {
	...
	try {
		...
		if (!isLoggableAttackType(obj.type)) return null;
...
async function processLogSlice(logPath, readStart, readLen, onLine) {
	return new Promise((resolve) => {
		const stream = createReadStream(logPath, {
			start: readStart,
			end: readStart + readLen - 1,
			encoding: "utf8",
		});
...
function attackTypeFromAutoBanNote(note) {
	const m = String(note ?? "").match(/^Auto-ban:\s*([a-z0-9_-]+)/i);
	if (!m) return null;
	const t = m[1].toLowerCase();
	return isValidAttackType(t) ? t : null;
}

async function visibleAttackTypeForEvent(knex, ev) {
	if (isValidAttackType(ev.type)) return ev.type;
	if (ev.type !== "deny") return null;

	try {
		const rule = await knex("nyxguard_ip_rule")
			.select("note")
			.where({ ip_cidr: ev.ip, action: "deny" })
			.orderBy("id", "desc")
			.first();
		const noteType = attackTypeFromAutoBanNote(rule?.note);
		if (noteType) return noteType;
	} catch {
		// ignore and fall back to historical event type
	}

	try {
		const last = await knex("nyxguard_attack_event")
			.select("attack_type")
			.where({ ip: ev.ip })
			.whereIn("attack_type", ["sqli", "ddos", "bot", "authfail"])
			.orderBy("created_on", "desc")
			.first();
		if (isValidAttackType(last?.attack_type)) return last.attack_type;
	} catch {
		// ignore
	}
...
async function pollOnce() {
	...
	// Rotation or truncation. If the inode changed but the saved byte offset still
	// exists in the current file, keep it; Docker volume remounts can change inode
	// metadata without replacing the log contents.
	if (st.size < offset || (state.inode !== inode && offset <= 0)) {
		offset = 0;
	}
	...
	const lines = [];
	const readOk = await processLogSlice(logPath, readStart, readLen, (line) => {
		lines.push(line);
	});
	if (!readOk) {
		return;
	...
	for (const line of lines) {
		...
		const eventType = await visibleAttackTypeForEvent(knex, ev);
		if (!eventType) continue;
		const isFreshDetection = isValidAttackType(ev.type);
		...
		await insertWebThreatEventForAttack(knex, { ...ev, type: eventType });

		try {
			if (isFreshDetection && shouldAutoBan(eventType)) {
				...
				if (!ev.auth) {
								...
								: eventType === "sqli"
									...
									: eventType === "authfail"
										...
										: Number.parseInt(process.env.NYXGUARD_AUTOBAN_THRESHOLD ?? "", 10) ||
											DEFAULT_AUTOBAN_THRESHOLD;
					const windowSec =
						eventType === "bot" || eventType === "sqli" || eventType === "ddos"
							? 1
							: eventType === "authfail"
					...
					if (recordAndShouldBan(eventType, ev.ip, ev.tsMs, { threshold, windowSec })) {
						const ban = await upsertAutoBanRule(knex, ev.ip, eventType, banUntil, ev.host);
						if (ban.changed) {
							banChanged = true;
							pendingReload = true;
							await applyPendingReload(knex);
	...
	try {
		...
		// Expired/disabled attack-derived bans should disappear; other temporary rules are only disabled.
		try {
			const deletedAttackDerived = await internalNyxGuard.ipRules.pruneInactiveAttackDerivedRules(knex);
			...
			if (deletedAttackDerived > 0 || expiredRuleIds.length > 0) banChanged = true;
		} catch {
			// ignore
		}
...
async function runPollOnce() {
	// setInterval does not wait for an async callback. Keep the monitor single-flight
	// so a slow poll cannot fill the database pool with overlapping state updates.
	if (pollInFlight) return;
	pollInFlight = true;
	try {
		await pollOnce();
	} catch {
		// Preserve the existing best-effort polling behavior.
	} finally {
		pollInFlight = false;
	}
}
