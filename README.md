...
```bash
curl -fsSL https://raw.githubusercontent.com/NyxCloudRO/NyxGuardManager/main/install.sh | sudo APP_VERSION=3.0.3 bash
```
...
- Install a specific version: `APP_VERSION=3.0.3`
...
`docker-compose.yml` is pinned to `nyxmael/nyxguardmanager:3.0.3` for production installs. To use a different version, edit the `image:` tag in `docker-compose.yml`.
...
- If you want to update to a specific release, change the `image:` tag in `docker-compose.yml` (for example, `nyxmael/nyxguardmanager:3.0.3`) before running `docker compose pull`.
