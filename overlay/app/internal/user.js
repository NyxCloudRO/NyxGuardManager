...
function generateRecoveryKey() {
	// 64 hex chars (256-bit) + prefix to reduce copy/paste confusion.
	return `NGM-REC-${crypto.randomBytes(32).toString("hex").toUpperCase()}`;
}
...
const internalUser = {
	 ...
	 * @returns {Promise}
	 */
	create: async (access, data, options = {}) => {
		const emitRecoveryKey = options?.emitRecoveryKey === true;
		...
		if (auth) {
			...
			if (auth.type === "password") {
				...
				authMeta = {
					...
					recovery_key_issued_on: new Date().toISOString(),
				};
			}
		...
		await userPermissionModel.query().insert({
			...
			redirection_hosts: isAdmin ? "manage" : "hidden",
			dead_hosts: isAdmin ? "manage" : "hidden",
			streams: isAdmin ? "manage" : "hidden",
			access_lists: isAdmin ? "manage" : "view",
			certificates: isAdmin ? "manage" : "view",
			nyxguard: isAdmin ? "manage" : "view",
			web_controls: isAdmin ? "manage" : "view",
			users: isAdmin ? "manage" : "view",
			auditlog: isAdmin ? "manage" : "view",
			settings: isAdmin ? "manage" : "view",
		});

		user = await internalUser.get(access, { id: user.id, expand: ["permissions"] });
		if (emitRecoveryKey && recoveryKey) {
