...
// Defensive: some reverse-proxy + strict-routing combinations can result in the
// mounted sub-router receiving a path without the leading "/". Normalize so
// route matchers like "/effective" still work.
...
router.get("/effective", jwtdecode(), async (req, res, next) => {
	try {
		await res.locals.access.can("web_controls:get");
...
router.post("/policy-sets/:id/versions", jwtdecode(), async (req, res, next) => {
	try {
		await res.locals.access.can("web_controls:update");
		...
		const created = await internalWebThreatControls.createPolicyVersion(db(), id, policy, createdBy, {
